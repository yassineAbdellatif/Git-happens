...
// --- Extracted sub-components to reduce MapScreen cognitive complexity ---
...
const OriginPicker: React.FC<OriginPickerProps> = ({
  originType,
  destinationLabel,
  userLocation,
  setOrigin,
}) => {
  if (originType === "BUILDING") {
    return (
      <>
        <Text style={styles.routingTitle}>Select origin building</Text>
        <ScrollView style={{ maxHeight: 180 }}>
          {CONCORDIA_BUILDINGS.filter((b) => b.fullName !== destinationLabel).map((b) => (
            <TouchableOpacity
              key={b.id}
  ...
  return (
    <>
      ...
      <TouchableOpacity
        style={[styles.originOptionButton, styles.primaryOptionButton]}
        onPress={() => {
          if (userLocation) {
            setOrigin({
              type: "CURRENT",
      ...
      >
        <View style={[styles.optionIconBadge, { backgroundColor: "rgba(66, 133, 244, 0.15)" }]}>
          <MaterialIcons name="my-location" size={22} color="#4285F4" />
        </View>
        <View style={styles.optionTextWrapper}>
          <Text style={[styles.originOptionText, { color: "#1f4b99", fontWeight: "700" }]}>
            Use current location
          </Text>
          <Text style={styles.optionSubtext}>Start from where you are right now</Text>
        </View>
      </TouchableOpacity>

      <TouchableOpacity
        style={styles.originOptionButton}
        onPress={() => setOrigin((prev: any) => ({ ...prev, type: "BUILDING" }))}
      >
        <View style={[styles.optionIconBadge, { backgroundColor: "rgba(145, 35, 56, 0.1)" }]}>
          <MaterialIcons name="domain" size={22} color="#912338" />
        </View>
        <View style={styles.optionTextWrapper}>
          <Text style={styles.originOptionText}>Choose a building</Text>
          <Text style={styles.optionSubtext}>Select from a list of campus buildings</Text>
        </View>
      </TouchableOpacity>
    </>
  );
...
const DestinationPicker: React.FC<DestinationPickerProps> = ({
  ...
  if (destinationType === "BUILDING") {
    return (
      <>
        <Text style={styles.routingTitle}>Select destination building</Text>
        <ScrollView style={{ maxHeight: 180 }}>
          {CONCORDIA_BUILDINGS.filter((b) => b.fullName !== originLabel).map((b) => (
            <TouchableOpacity
              key={b.id}
              style={styles.dropdownItem}
              onPress={() =>
  ...
  return (
    <>
      <Text style={styles.routingTitle}>Select Destination</Text>
      <TouchableOpacity
        style={[styles.originOptionButton, styles.primaryOptionButton]}
        onPress={() => {
          if (userLocation) {
      ...
      <TouchableOpacity
        style={styles.originOptionButton}
        onPress={() => setDestination((prev: any) => ({ ...prev, type: "BUILDING" }))}
      >
        <View style={[styles.optionIconBadge, { backgroundColor: "rgba(145, 35, 56, 0.1)" }]}>
          <MaterialIcons name="domain" size={22} color="#912338" />
        </View>
...
const getCampusLabel = (
  currentBuilding: { campus: string } | null,
  isCloserToSgw: boolean,
): string => {
  if (currentBuilding) return currentBuilding.campus;
  return isCloserToSgw ? "SGW" : "LOYOLA";
};
...
const MapScreen = () => {
  ...
  const campusLabel = getCampusLabel(currentBuilding, isCloserToSgw);
        ...
        {!isNavigating && (
          <View style={styles.rightControlsContainer}>
            ...
            <TouchableOpacity
              ...
              style={[styles.nearbyPoiButton, poi.isOpen && styles.nearbyPoiButtonOpen]}
            ...
            >
              {selectedBuilding && !isRouting ? (
                <View style={{ width: "100%" }}>
                  ...
                  <Text style={styles.buildingInfo}>{selectedBuilding.info}</Text>
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      ...
                      setDestination({ type: null, coords: null, label: "Select Destination", campus: null });
                      setIsRouting(true);
                    }}
                  >
                    <MaterialIcons name="my-location" size={20} color="white" style={{ marginRight: 8 }} />
                    <Text style={styles.directionsButtonText}>Start from here</Text>
                  </TouchableOpacity>
                  ...
                  >
                    <MaterialIcons name="place" size={20} color="white" style={{ marginRight: 8 }} />
                      ...
                      {transportMode === "SHUTTLE" && nextShuttleTitle.length > 0 && (
                        <View style={styles.shuttleInfo}>
                          ...
                          {nextShuttleSubtitle.length > 0 && (
                            <Text style={styles.shuttleSubtext}>{nextShuttleSubtitle}</Text>
                          )}
                        </View>
                      )}
          ...
          <View style={styles.navigationSheet}>
            ...
            <View style={styles.routeSummary}>
              <View style={styles.summaryRow}>
                <MaterialIcons
                  name={(MODE_ICON_MAP[transportMode as keyof typeof MODE_ICON_MAP] || "directions-walk") as any}
                  size={24}
                  color="#912338"
                />
