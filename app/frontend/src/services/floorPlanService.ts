...
function resolveFloor(
  buildingId: IndoorBuildingId,
  rawBuildingId: string,
  rawFloor: number | string,
  nodeId: string,
): FloorNumber {
  const floor = normalizeFloorNumber(buildingId, rawBuildingId, rawFloor);
  if (VALID_FLOORS.has(floor)) return floor;
  console.warn(
    `Unknown floor "${String(rawFloor)}" for node "${nodeId}", falling back to "1"`,
  );
  return "1";
}
...
function toLocalizedNode(node: RawMapNode): LocalizedNode {
  const normalizedBuildingId =
    normalizeBuildingId(String(node.buildingId)) || "H";
...
function matchesBuildingId(
  ...
  const normalized = normalizeBuildingId(String(node.buildingId));
  return normalized === buildingId;
}
...
export const getFloorPlanRegistryEntry = (
  ...
  const rawNodesForFloor = ALL_RAW_NODES.filter((node) => {
    if (!matchesBuildingId(node, buildingId)) {
      return false;
    }

    const normalizedBuildingId =
      normalizeBuildingId(String(node.buildingId)) || buildingId;
    const normalizedFloor = normalizeFloorNumber(
      normalizedBuildingId,
      String(node.buildingId),
      node.floor,
    );

    return normalizedFloor === floorNumber;
  });
